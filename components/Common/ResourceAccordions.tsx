...
function ResourceAccordions({ page }: { page: PageType }) {
  ...
  const { data: accData, isLoading, error } = useResources().getAccordionData(page, !!page);
  ...
  // Show loading state
  if (isLoading) {
    return <div className="flex flex-col items-start justify-start gap-4 w-full">Loading resources...</div>;
  }

  // Show error state
  if (error) {
    return <div className="flex flex-col items-start justify-start gap-4 w-full">Error loading resources. Please try again later.</div>;
  }
